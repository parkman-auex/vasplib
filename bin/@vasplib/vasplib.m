    ...
    methods(Static)
        ...
        function [kpoints,nodes,kpoints_name,kpoints_name_pre] = KPOINTS_read(filename,mode)
            ...
            if strcmp(mode,'vasp')
                ...
                kpoints_name_t = strrep(kpoints_name_t,'GAMMA','Γ');
                kpoints_name_t = strrep(kpoints_name_t,'Gamma','Γ');
                kpoints_name_t = strrep(kpoints_name_t,'G','Γ');
                ...
                error(fprintf('Unsupport mode:%s \n',mode));
            end
            %-------- return --------
        end
...
%                 Smat_inv = inv(Smat);
                ...
                for i = 1:numel(H_sym_L)
                    Label_tmp = find(Smat(i,:));
                    H_sym_pauli_L(i) = H_sym_L(Label_tmp)*Smat(i,Label_tmp)';
                end
