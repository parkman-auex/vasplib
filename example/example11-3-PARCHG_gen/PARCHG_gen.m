function [ PARCHGCAR , PARCHG_line ]= PARCHG_gen(orb_list,WaveFunc,PARCHG_file,curve_R,Rm,sites,Atom_name,Atom_num )
% PARCHG gen
...
if strcmp(mode,'glance')
    ...
    for i = 1:Norb
        ...
        WFamp = temp_C;
        WFplot_list(i,:) = [orb_list(i,:)*Rm ,WFamp];
    end
    figure();
    for i = 1 : Norb
        ...
        if WFplot_list(i,4) > 0
            plot3(WFplot_list(i,1),WFplot_list(i,2),WFplot_list(i,3),'ro','MarkerSize',(WFplot_list(i,4))*1000,'MarkerFaceColor','r');
        else
            plot3(WFplot_list(i,1),WFplot_list(i,2),WFplot_list(i,3),'ko','MarkerSize',1,'MarkerFaceColor','k');
        end
        hold on
    end
                    ...
                    if jy >=1 && jy <= NGYF
                        for ix = Ro(1)-adjustparmX : Ro(1)+adjustparmX+1
                            if ix >=1 && ix <= NGXF
                                ...
                                [add_orb,Rlength] = check_orbital_inrange(PARCHG_line(seq).Rc,temp_orb,Rm);
                                if add_orb == 1
                                    %disp('bug_fix');
