...
%% origin
tic;
KaneMele_trig = KaneMele.HR2Htrig();
KaneMele_trig_n = KaneMele_trig.Subsall();
%KaneMele_trig_n = Kane_Mele_n.HR2Htrig();
EIGENCAR = KaneMele_trig_n.EIGENCAR_gen();
bandplot(EIGENCAR ,[-3,3]);
toc;
%% direct 
tic;
KaneMele_trig = KaneMele.HR2Htrig('direct',true);
KaneMele_trig_n = KaneMele_trig.Subsall();
EIGENCAR = KaneMele_trig_n.EIGENCAR_gen();
bandplot(EIGENCAR ,[-3,3]);
toc;
%% fastest For HR2Htrig
KaneMele_trig_n = Kane_Mele_n.HR2Htrig('fast',true,'Type','list');
%KaneMele_trig_n = KaneMele_trig.Subsall();
tic;
EIGENCAR = KaneMele_trig_n.EIGENCAR_gen();
bandplot(EIGENCAR ,[-3,3]);
...
toc;
%%
simplify(KaneMele_trig.Htrig_sym,'IgnoreAnalyticConstraints',true)
%% 参考文献
