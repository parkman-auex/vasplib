...
function [H,H_xyz,orb_list]=H_TB_gen2(level_cut,nn_store,Rm,per_dir)
%% file=
disp("H_TB")
%%
...
%   on-site
    for j=1:N_orbit
         H(j,j)="E_onsite_"+string(j);
         EVALstring="syms E_onsite_"+string(j)+" real;";
         eval(EVALstring);
    ...
    for j=1:N_orbit
        for i=1:N_tot
            ...
            if level <= level_cut
                ...
                %disp(i);
                %disp(j);
                count = level_cut*(nn_store(i,j).ion_num-1)+level ;
