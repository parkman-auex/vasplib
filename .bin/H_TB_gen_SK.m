%% To gen H_TB.m(use syms) with Rnn 
...
function [H,H_xyz,orb_list]=H_TB_gen_SK(level_cut,nn_store,Rm,per_dir)
%% file=
disp("H_TB")
%%
...
%% term_storage
    ...
    % syms t_a t_b t_c t_d t_e t_f t_g t_i t_j t_k t_l t_m real;
    
    syms k k_x k_y k_z real
    syms E_onsite real
    % t_store=["t_a","t_b","t_c","t_d","t_e","t_f","t_g","t_f","t_i","t_j","t_k","t_l","t_m"];
...
%   on-site
    for j=1:N_orbit
         H(j,j)="E_onsite_"+nn_store(j,j).nameseq;
...
%   hop
    for j=1:N_orbit
        fprintf("setting (%d/%d)th orbital ... \n",j,N_orbit);
        for i=1:N_tot
            ...
            if level <= level_cut
                ...
                %fix bug here
                H_xyz = set_hop(-nn_store(i,j).hop,j,k,vector,H_xyz,'sym');
            end
        end
    end
end
