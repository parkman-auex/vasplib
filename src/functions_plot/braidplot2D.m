function varargout=braidplot2D(EIGENCAR,Ecut,klist_l,kpoints_l,kpoints_name,options)
arguments
...
%         kpoints_name=kpoints_name;
    ...
    if options.set_reference
        ...
        if options.set_plane
            ...
            for i = 1:length(kpoints_l)
                V{i} = [
                    Xcut(1),Ycut(1),kpoints_l(i);
                    Xcut(1),Ycut(2),kpoints_l(i);
                    Xcut(2),Ycut(2),kpoints_l(i);
                    Xcut(2),Ycut(1),kpoints_l(i);
                    ];
                patch('Faces',[1,2,3,4],'Vertices',V{i},...
                    'EdgeColor','none',...
    ...
    for Ei=1:Nbands
        plot3(ax,RealEIGENCAR(Ei,:),ImagEIGENCAR(Ei,:),klist(1:nKinEIGENCAR),options.LineSpec,...
    ...
    if options.set_plane
        ...
        for i = 1:length(kpoints_l)
            ...
            if ~isempty(SelectL)
                try
                    scatter3(ax,RealEIGENCAR(:,SelectL(1)),ImagEIGENCAR(:,SelectL(1)),ones(1,Nbands)*kpoints_l(i),...
